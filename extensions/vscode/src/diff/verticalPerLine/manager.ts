...
export class VerticalPerLineDiffManager {
  ...
  // Listener for user doc changes is disabled during updates to the text document by softcodes
  ...
  clearForFilepath(filepath: string | undefined, accept: boolean) {
    ...
    vscode.commands.executeCommand("setContext", "softcodes.diffVisible", false);
  }
  ...
  async acceptRejectVerticalDiffBlock(
    ...
    // Disable listening to file changes while softcodes makes changes
    this.disableDocumentChangeListener();
  ...
  async streamEdit(
    ...
    vscode.commands.executeCommand("setContext", "softcodes.diffVisible", true);
    ...
    if (existingHandler) {
      ...
      // but extra delta was added for each line generated by Softcodes
      // Before adding this back, we need to distinguish between human and Softcodes
