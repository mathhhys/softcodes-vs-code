# Softcodes VS Code Extension

This is the Softcodes VS Code Extension. Its primary jobs are

1. Implement the IDE side of the Softcodes IDE protocol, allowing a Softcodes server to interact natively in an IDE. This happens in `src/softcodesIdeClient.ts`.
2. Open the Softcodes React app in a side panel. The React app's source code lives in the `gui` directory. The panel is opened by the `continue.openContinueGUI` command, as defined in `src/commands.ts`.
